...
@login_required
def success():
    new_paid_chat = PaidChats(user=session['email'], doctor_email=session['doc_email'])
    db.session.add(new_paid_chat)
    
    new_client = DoctorClient(doctor_email=session['doc_email'], client_email=session['email'], status="in progress")
